...
public class MovieDao extends AbstractMFlixDao {
	...
	public Document getMovie(String movieId) {
		...
		List<Bson> pipeline = Arrays
				.asList(new Document("$match", new Document("_id", new ObjectId(movieId))),
						new Document("$lookup",
								new Document("from", "comments").append("let", new Document("id", "$_id"))
										.append("pipeline", Arrays.asList(new Document("$match",
												new Document("$expr",
														new Document("$eq", Arrays.asList("$movie_id", "$$id"))))))
										.append("as", "comments")),
						new Document("$unwind", new Document("path", "$comments")),
						new Document("$sort", new Document("comments.date", -1L)),
						new Document("$group",
								new Document("_id", "$_id").append("fullplot", new Document("$first", "$fullplot"))
										.append("imdb", new Document("$first", "$imdb"))
										.append("year", new Document("$first", "$year"))
										.append("plot", new Document("$first", "$plot"))
										.append("genres", new Document("$first", "$genres"))
										.append("rated", new Document("$first", "$rated"))
										.append("metacritic", new Document("$first", "$metacritic"))
										.append("title", new Document("$first", "$title"))
										.append("lastupdated", new Document("$first", "$lastupdated"))
										.append("languages", new Document("$first", "$languages"))
										.append("writers", new Document("$first", "$writers"))
										.append("type", new Document("$first", "$type"))
										.append("tomatoes", new Document("$first", "$tomatoesr"))
										.append("poster", new Document("$first", "$poster"))
										.append("num_mflix_comments", new Document("$first", "$num_mflix_comments"))
										.append("released", new Document("$first", "$released"))
										.append("awards", new Document("$first", "$awards"))
										.append("countries", new Document("$first", "$countries"))
										.append("cast", new Document("$first", "$cast"))
										.append("directors", new Document("$first", "$directors"))
										.append("runtime", new Document("$first", "$runtime"))
										.append("comments", new Document("$push", "$comments"))));

		return moviesCollection.aggregate(pipeline).first();
		/*
		 * Bson lookup = new Document("$lookup", new Document("from",
		 * "comments").append("localField", "_id") .append("foreignField",
		 * "movie_id").append("as", "comments")); List<Bson> pipeline = new
		 * ArrayList<>(); // match stage to find movie Bson match =
		 * Aggregates.match(Filters.eq("_id", new ObjectId(movieId)));
		 * pipeline.add(match); pipeline.add(lookup);
		 * 
		 * Bson sort = Aggregates.sort(Sorts.ascending("date")); pipeline.add(sort); //
		 * TODO> Ticket: Get Comments - implement the lookup stage that allows the //
		 * comments to // retrieved with Movies. Document movie =
		 * moviesCollection.aggregate(pipeline).first();
		 * 
		 * return movie;
		 */
	}
	...
	public List<Document> getMoviesByCast(String sortKey, int limit, int skip, String... cast) {
		...
		// new Document("cast", new Document("$all", Arrays.asList(cast)));
		Bson sort = Sorts.descending(sortKey);

		// new Document(sortKey, 1);
